"""
Copyright 2019 groud0state. All rights reserved.
"""
import numpy as np
import unittest
...
class DTW():
    ...
    def warp_distance(self, ts_1, ts_2, d=lambda x, y: np.abs(x-y)):
        ...
        y = 0
        self.warp_path = []
        self.warp_path.append([0, 0])
        while (x < ts_1_len - 1 and y < ts_2_len - 1):
            if dist[x + 1][y + 1] <= dist[x + 1][y] and dist[x + 1][y + 1] <= dist[x][y + 1]:
                x += 1
                y += 1
            elif dist[x][y + 1] <= dist[x + 1][y]:
                y += 1
            else:
                x += 1

            self.warp_path.append([x, y])
...
class TestDTW(unittest.TestCase):
    def test_distance(self):
        ...
        dtw = DTW()
        distance = dtw.warp_distance(ts_1, ts_2)
